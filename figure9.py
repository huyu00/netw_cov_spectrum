...
from cov_spectrum_random_netw import (pdf_g, pdf_g_x, cdf_g, fit_cdf_g,
    ...
    pdf_P_g_kre_x, fit_cdf_g, pdf_g_f, fit_cdf_g_f, fit_cdf_g_kre)
...
for t in range(niter):
    gh, s02h, cost = fit_cdf_g(eig_C[tf_in], cost='CvM')
    x12h = support_g(gh)
...
plt.ylim(bottom=0)
plt.tight_layout()
fig.savefig('./figure/figure9_6', dpi=600, transparent=True)

fig = plt.figure(figsize=(8,5))
plt.plot(range(N), eig_C[::-1],'.-',markersize=10)
plt.tight_layout()
fig.savefig('./figure/figure9_6b', dpi=600, transparent=True)


# test across trials
ntrial = 10 # use 100 trials for more stable results
g = 0.6
s02 = 1
s12 = 17
s22 = 15
...
print('fraction correct 1:', n_correct1/ntrial)
...
plt.hist(gh_ls-g,30)
plt.tight_layout()
fig.savefig('./figure/figure9_noninter_g_error', dpi=600, transparent=True)
# no-iteration fitting MSE: 0.0321 or 5.35%
# with open('./data/figure9_noninter_g_error.npy', 'wb') as file1:
#     np.save(file1, gh_ls)
